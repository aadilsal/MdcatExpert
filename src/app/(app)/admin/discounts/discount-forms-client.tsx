...
export function TogglePromoForm({ promoId, isActive }: { promoId: string; isActive: boolean }) {
    return (
        <form action={togglePromoCodeActiveAction} className="inline-flex">
            ...
            <FormSubmitButton className="inline-flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700 bg-surface px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition disabled:opacity-60 disabled:cursor-not-allowed">
                {isActive ? (
                    <>
...
export function CreatePromoForm() {
    return (
        <form action={createPromoCodeAction} className="space-y-5">
            <div>
                <label htmlFor="code" className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Code
                </label>
                <input
                    id="code"
                    ...
                    className="theme-input mt-2 w-full rounded-3xl px-4 py-3 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:focus:ring-primary-900/40"
                />
            </div>
            <div>
            ...
            <div>
                <label htmlFor="maxUses" className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                    Max Uses
                </label>
                <input
                    id="maxUses"
