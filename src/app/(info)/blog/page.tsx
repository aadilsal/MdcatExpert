...
export default async function BlogPage() {
  ...
  return (
    <div className="space-y-16">
      ...
      {posts.length === 0 ? (
        <div className="text-center py-20 rounded-3xl bg-gray-50 dark:bg-slate-900/60 border border-gray-100 dark:border-slate-800">
          <p className="text-gray-500 dark:text-slate-400 font-medium">New articles coming soon. Check back shortly!</p>
        </div>
      ) : (
        <div className="grid gap-8">
          {posts.map((post) => (
            <article
              key={post._id}
              className="group p-8 rounded-3xl bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 hover:border-primary-100 dark:hover:border-primary-800 hover:shadow-xl hover:shadow-primary-600/5 transition-all"
            >
              <div className="flex flex-wrap items-center gap-3 mb-4">
                {post.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 rounded-full bg-primary-50 dark:bg-primary-950/40 text-primary-700 dark:text-primary-400 text-[10px] font-black uppercase tracking-widest"
                  >
                    {tag}
                  </span>
                ))}
                <span className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-slate-500 font-bold">
                  <Calendar className="w-3.5 h-3.5" />
                  {formatDate(post.publishedAt ?? post.createdAt)}
                </span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mb-3 group-hover:text-primary-600 transition-colors">
                <Link href={`/blog/${post.slug}`}>{post.title}</Link>
              </h2>
              <p className="text-gray-500 dark:text-slate-400 font-medium leading-relaxed mb-6">{post.excerpt}</p>
              <Link
                href={`/blog/${post.slug}`}
